...
两大类 Actions, Services

三小类 Commands, Options, and Imports

使用 # 注释
使用\\续行
使用“” 带空格的字符串

Actions and Services必须唯一。  
如果 Action 重名了则其commands续接到先定义的action上；
如果 Service 重名了，则后则会被忽略且有错误报出。
...
early" and "late" in mount_all command
